...
/**
 * Snippet for producing Lift notices
 */
object Notices extends Factory {
        ...
        notices(messages).toList match {
          ...
          case msgs if (msgs.length == 1) =>
            <div id={noticeType.id} class={"alert-message %s".format(lowerCaseTitle(noticeType))} data-alert=" ">
              <p>
                {noticeTitle(noticeType).map(t => <strong>
                {t}
              </strong>).openOr(Text(""))}{msgs(0)}
              </p>
            </div>
          case msgs =>
            <div id={noticeType.id} class={"alert-message %s".format(lowerCaseTitle(noticeType))} data-alert=" ">
              {noticeTitle(noticeType).map(t => <strong>
              {t}
            </strong>).openOr(Text(""))}{msgs.flatMap(e => {
              <p>
                {e}
              </p>
            })}
